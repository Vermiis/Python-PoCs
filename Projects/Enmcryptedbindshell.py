...
def client(ip):
    ...
    threading.Thread(target=send_thred, args=(s, )).start()
    threading.Thread(target=recv_thread, args=(s,)).start()

parser = argparse.ArgumentParser()
parser.add_argument("-l", "--listen", dest="listen", action="store_true", help="Setup a listener", required=False)
parser.add_argument("-c", "--connect", action="store_true", help="Connect to a shell", required=False)
parser.add_argument("-k", "--key", help="Encrypt key", type = str, required=False)
args=parser.parse_args()


if args.connect and not args.key:
    parser.error("gib key")
